...
function change_link(link_id, select_id)
{
    ...
    switch(option)
    {
        ...
        case 1:
        {
            ...
            auto_ship		= "<br>(Enviar Automaticamente cada 30 dias)";
        ...
        case 2:
        {
            ...
            auto_ship		= "<br>(Enviar Automaticamente cada 2 meses)";
        ...
        case 3:
        {
            ...
            auto_ship		= "<br>(Enviar Automaticamente cada 3 meses)";
        ...
        case 4:
        {
            ...
            auto_ship		= "<br>(Enviar Automaticamente cada 6 meses)";
    ...
    var position		= array_descr.search(/<br(\/)?>\(Enviar Automaticamente cada \d+ (dias|meses)\)/gi);
    array_descr			= array_descr.replace(/<br(\/)?>\(Enviar Automaticamente cada \d+ (dias|meses)\)/gi, "");
    if(position != -1)
    {
...
function reset_cart_link(link, orig_price, orig_weight, site_discount, discount_select_exists) {
    ...
    //Acá se quita la parte que dice "Combinado con...":
    array_descr     = array_descr.replace(/<br\/>\(Combinado con [\w\s]*\(?[\w\s]*?\)?\)/gi, "");
    //Y tambien la parte, si existe, de "Con/Sin Cajita":
    array_descr     = array_descr.replace(/<br\/>\(Capsulas (si|co)n caja\)/gi, "");
    ...
    if(discount_select_exists)
    {
        ...
        array_descr = array_descr.replace(/<br(\/)?>\(Con \d+% de descuento!\)/gi, "");
...
function get_comb_image(combine_this, with_this)
{
    ...
    switch(combine_this)
    {
        ...
        case "euf":                     //EUF con...
            switch(with_this)
            {
                ...
                case "vp_caps":         //...VP en capsulas
                    ...
                    break;
                case "parche_plus":     //...Parche Pro-Sero
                    image_link  = image_path + "euf+parche.png";
                    break;
                case "blue_cell":       //...Blue Cell
        ...
        case "vp_caps":                 //VP en capsulas con...
            ...
            break;
        case "blue_cell":               //Blue Cell Serum con...
            switch(with_this)
            {
                case "euf":             //...EUF
                    image_link  = image_path + "euf+bc.png";
                ...
                case "parche_plus":     //...Parche Pro-Sero
                    ...
                    break;
                case "blue_cell":       //...Blue Cell -- combinado consigo mismo.
            ...
            break;
        case "parche_plus":             //Parche Pro-Sero con...
            switch(with_this)
            {
                case "euf":             //...EUF
                    image_link  = image_path + "euf+parche.png";
                ...
                case "pe":              //...PE
                    image_link  = image_path + "pe+parche.png";
                    break;
                case "parche_plus":     //...Parche Pro-Sero -- combinado consigo mismo.
...
function change_combined(selected_option, 
    ...
    switch(selected_option)
    {
        ...
        case 6:
        {
            //Paquete Parche Pro-Sero
            if(DEBUG){
                alert("Parte 2.1, combinando con: Parche Pro-Sero.");
            }
        ...
        case 7:
        {
            //Blue Cell
            if(DEBUG){
                alert("Parte 2.1, combinando con: Blue Cell.");
            }
    ...
    if(selected_option != 0)
    {
        ...
        if(selected_option == 4 || selected_option == 5) {
            ...
            box_text = "<br/>("+((box_option == 0) ? "Capsulas sin caja" : "Capsulas con caja")+")";
        }
        //El producto queda entonces combinado -- solo colocar si no lo está previamente:
        if(!/combinado/gi.test(array_descr)) {
            array_descr         = array_descr + "<br/>(Combinado con " + combined_descr + ")";
        }
    }
    if (box_text != "") {
                        ...
                        )
{
    ...
    if( percentage != 0 )
    {
        if(quantity != 1)
        {
            array_descr 		= quantity + "x " + array_descr + "<br>(Con " + percentage + "% de descuento!)";
        }
        else if(site_discount != 1)
        {
            percentage			= ((1 - site_discount) * 100).toFixed(0);
            array_descr 		= array_descr + "<br>(Con " + percentage + "% de descuento!)";
            discount			= site_discount;
        }
    }
...
function change_select_2(select_id, change_to)
{
    ...
    switch(change_to)
    {
        case "vp_caps":
        {
            ...
            select.options[select.length]   = new Option("Combinar con VP (30 caps)", 
                                                         5, 
                                                         false, 
                                                         option_5);
        ...
        {		
            ...
            select.options[select.length]   = new Option("Combinar con PE (30 caps)", 
                                                         4, 
                                                         false, 
                                                         option_4);
            //Solo si la ultima opcion del select anterior es distinta a las ya colocadas, 
...
function capsule_type(link, change_to)
{
    ...
    //Esta es una opcion provisional, para los productos Blue Cell, Parche y Kimi
    var reg_exp_BC          = /Blue Cell/gi;
    var reg_exp_parche      = /Parche Pro-Sero/gi;
    //Esto es para los tipos de aplicacion de producto:
    var reg_exp_caps        = /en Capsulas/i;
    var reg_exp_drink       = /Bebible/i;
    ...
    switch(parseInt(change_to))
    {
        ...
        {   
            ...
            {
                if(DEBUG) {
                    alert("El producto seleccionado es PE y actualmente es Blue Cell");
                }		
            ...
            {
                if(DEBUG) {
                    alert("El producto seleccionado es PE y actualmente es Parche Pro-Sero");
                }		
            ...
            if(reg_exp_drink.test(array_descr))
            {
                ...
                array_descr = array_descr.replace(reg_exp_drink, "en Capsulas");
            }
            if(reg_exp_patch.test(array_descr))
            {
                ...
                array_descr = array_descr.replace(reg_exp_patch, "en Capsulas");
            }
            break;
        }
        case 2:
        {
            ...
            {
                if(DEBUG) {
                    alert("El producto seleccionado es VP y actualmente es Blue Cell");
                }		
            ...
            {
                if(DEBUG) {
                    alert("El producto seleccionado es VP y actualmente es Parche Pro-Sero");
                }		
        ...
        case 6: //El caso particular del Parche
        {
            ...
            {
                if(DEBUG)	{
                    alert("El producto seleccionado es Parche Pro-Sero y actualmente es PE");
                }
                array_descr = array_descr.replace(reg_exp_PE, "Parche Pro-Sero");
            }
            if(reg_exp_VP.test(array_descr))
            {
                if(DEBUG) {
                    alert("El producto seleccionado es Parche Pro-Sero y actualmente es VP");
                }		
                array_descr = array_descr.replace(reg_exp_VP, "Parche Pro-Sero");
            }
            if(reg_exp_BC.test(array_descr))
            {
                if(DEBUG) {
                    alert("El producto seleccionado es Parche Pro-Sero y actualmente es Blue Cell");
                }		
                array_descr = array_descr.replace(reg_exp_BC, "Parche Pro-Sero");
            }
            //Cambiar la forma de consumo:
        ...
        case 7: //El caso particular del Blue Cell
        {
            if(reg_exp_PE.test(array_descr))
            {
                if(DEBUG)	{
                    alert("El producto seleccionado es Blue Cell y actualmente es PE");
                }
                array_descr = array_descr.replace(reg_exp_PE, "Blue Cell");
            }
            if(reg_exp_VP.test(array_descr))
            {
                if(DEBUG) {
                    alert("El producto seleccionado es Blue Cell y actualmente es VP");
                }		
                array_descr = array_descr.replace(reg_exp_VP, "Blue Cell");
            }
            if(reg_exp_parche.test(array_descr))
            {
                if(DEBUG) {
                    alert("El producto seleccionado es Blue Cell y actualmente es Parche Pro-Sero");
                }		
                array_descr = array_descr.replace(reg_exp_parche, "Blue Cell");
            }
            //Cambiar la forma de consumo:
            if(reg_exp_caps.test(array_descr))
            {
...
function update_product(product_type,
    ...
    if (type_select != null) {
        ...
        //Si es 6, es Parche Pro-Sero
        else if (type_option == '6') {
            if(DEBUG) {
                alert("Colocando el producto base como Parche Pro-Sero");
            }
            orig_product	= "parche_plus";
            link                = capsule_type(link, type_option);
            orig_price          = getBasePrice(product_type, orig_product, site_discount);
        }
        //Si es 7, es Blue Cell
        else if (type_option == '7') {
            if(DEBUG) {
                alert("Colocando el producto base como Blue Cell");
            }
