/* Algoritmo de cambio de producto:
    
    Si se mueve el primer select:
        Cambiar el contenido del segundo
        
    Si se mueve el segundo select:
        Cambiar el contenido del tercero
        
    Si se mueve el tercer select:
        Cambiar imagen del producto
        Cambiar descripcion del paquete
...
function setTypeSelect(currentPackage, vial_type, idNumber) {
    ...
    switch (vial_type){
        case 'euf': {
            sel_option[0]   = "Estandar: EUF ("+ 1*baseEUF +" ampollas)";
            sel_option[1]   = 0;
            select_opts.push(sel_option);
            sel_option      = new Array(2);
            sel_option[0]   = "Premium: EUF ("+ 2*baseEUF +" ampollas)";
            sel_option[1]   = 1;
            select_opts.push(sel_option);
            sel_option      = new Array(2);
            sel_option[0]   = "Optimum: EUF ("+ 3*baseEUF +" ampollas)";
            sel_option[1]   = 2;
            select_opts.push(sel_option);
            sel_option      = new Array(2);
            sel_option[0]   = "Optimum Plus: EUF ("+ 4*baseEUF +" ampollas)";
            sel_option[1]   = 3;
            select_opts.push(sel_option);
            break;
        }
        case 'hp': {
            sel_option[0]   = "Estandar: "+ 1*baseHP +" ampollas HP";
            sel_option[1]   = 4;
            select_opts.push(sel_option);
            sel_option      = new Array(2);
            sel_option[0]   = "Premium: "+ 2*baseHP +" ampollas HP";
            sel_option[1]   = 5;
            select_opts.push(sel_option);
            sel_option      = new Array(2);
            sel_option[0]   = "Optimum: "+ 3*baseHP +" ampollas HP";
            sel_option[1]   = 6;
            select_opts.push(sel_option);
            sel_option      = new Array(2);
            sel_option[0]   = "Optimum Plus: "+ 4*baseHP +" ampollas HP";
            sel_option[1]   = 7;
            select_opts.push(sel_option);
            break;
        }
        case 'vp': {
            sel_option[0]   = "Estandar: "+ 1*baseVP +" ampollas VP";
            sel_option[1]   = 8;
            select_opts.push(sel_option);
            sel_option      = new Array(2);
            sel_option[0]   = "Premium: "+ 2*baseVP +" ampollas VP";
            sel_option[1]   = 9;
            select_opts.push(sel_option);
            sel_option      = new Array(2);
            sel_option[0]   = "Optimum: "+ 3*baseVP +" ampollas VP";
            sel_option[1]   = 10;
            select_opts.push(sel_option);
            sel_option      = new Array(2);
            sel_option[0]   = "Optimum Plus: "+ 4*baseVP +" ampollas VP";
            sel_option[1]   = 11;
            select_opts.push(sel_option);
            break;
        }
...
function setCombineSelect(package_type, vial_type, idNumber) {
    ...
    switch (vial_type) {
        case 'euf': {
            switch (package_type) {
                case 'standard': {
                    sel_option[0]   = "Estandar: "+ 1*baseEUF +
                    " ampollas EUF + "+ 1*baseCombEUF +" cajitas MFIII PE";
                    sel_option[1]   = 1;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
                case 'premium': {
                    ...
                    " ampollas EUF + "+ 2*baseCombEUF +" cajitas MFIII PE";
                    sel_option[1]   = 2;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
                case 'optimum': {
                    ...
                    " ampollas EUF + "+ 3*baseCombEUF +" cajitas MFIII PE";
                    sel_option[1]   = 3;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
                case 'optimum_plus': {
                    ...
                    " ampollas EUF + "+ 4*baseCombEUF +" cajitas MFIII PE";
                    sel_option[1]   = 4;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
        ...
        case 'hp': {
            switch (package_type) {
                case 'standard': {
                    sel_option[0]   = "Estandar: "+ 1*baseHP +
                    " ampollas HP + "+ 1*baseCombHP +" cajitas MFIII PE";
                    sel_option[1]   = 5;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
                case 'premium': {
                    ...
                    " ampollas HP + "+ 2*baseCombHP +" cajitas MFIII PE";
                    sel_option[1]   = 6;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
                case 'optimum': {
                    ...
                    " ampollas HP + "+ 3*baseCombHP +" cajitas MFIII PE";
                    sel_option[1]   = 7;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
                case 'optimum_plus': {
                    ...
                    " ampollas HP + "+ 4*baseCombHP +" cajitas MFIII PE";
                    sel_option[1]   = 8;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
        ...
        case 'vp': {
            switch (package_type) {
                case 'standard': {
                    sel_option[0]   = "Estandar: "+ 1*baseVP +
                    " ampollas VP + "+ 1*baseCombVP +" cajitas MFIII PE";
                    sel_option[1]   = 9;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
                case 'premium': {
                    ...
                    " ampollas VP + "+ 2*baseCombVP +" cajitas MFIII PE";
                    sel_option[1]   = 10;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
                case 'optimum': {
                    ...
                    " ampollas VP + "+ 3*baseCombVP +" cajitas MFIII PE";
                    sel_option[1]   = 11;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
                case 'optimum_plus': {
                    ...
                    " ampollas VP + "+ 4*baseCombVP +" cajitas MFIII PE";
                    sel_option[1]   = 12;
                    select_opts.push(sel_option);
                    sel_option      = new Array(2);
                }
...
function getDescription(baseProductType, combinedProductType){
    ...
    case 'vp_caps':
        ...
        break;
    case 'parche_plus':
        combinedStr += "Parche Pro-Sero";
        break;
    case 'blue_cell':
        combinedStr += "Blue Cell";
        break;
    case 'kimi':
        combinedStr += "Kimi";
        break;
...
function getTherapy(baseType, combinedType) {
    ...
    case 'pe':
    case 'vp_caps':
        combinedStr = "en c&aacute;psulas";
        break;
    case 'parche_plus':
    case 'blue_cell':
        combinedStr = "aplicable";
        break;
    case 'kimi':
        combinedStr = "Kimi";
        break;
...
function setTherapy(idNumber, setContent){
    ...
    usage.innerHTML = "(Terapia " + setContent + ")";
    return true;
}
...
function getUsage(packageType, productType, combinedType, isIdeal){
    ...
    case 'pe':
        ...
        basePresentation    = "c&aacute;psulas";
        break;
    case 'vp_caps':
        baseQty = 30;
        baseTypeStr         = "MFIII VP";
    ...
    case 'parche_plus':
        baseQty = 1;
        baseTypeStr         = "Parche Pro-Sero";
        basePresentation    = "cajas de parches";
        break;
    case 'blue_cell':
        baseQty = 1;
        baseTypeStr         = "Blue Cell";
        basePresentation    = "frascos";
        break;
    case 'kimi':
        baseQty = 1;
        baseTypeStr         = "Kimi";
        basePresentation    = "plantillas";
        break;
    }
    //Set correct adjetive and number of items for base presentation:
    baseProductTotalQty = baseMultiplier * baseQty;
    if(basePresentation == "ampollas")
        basePresentation = (baseProductTotalQty > 1) ? "ampollas" : "ampolla";
    if(basePresentation == "c&aacute;psulas")
        basePresentation = (baseProductTotalQty > 1) ? "c&aacute;psulas" : "c&aacute;psula";
    if(basePresentation == "frascos")
        basePresentation = (baseProductTotalQty > 1) ? "frascos" : "frasco";
    if(basePresentation == "plantillas")
        basePresentation = (baseProductTotalQty > 1) ? "plantillas" : "plantilla";
    if(basePresentation == "cajas de parches")
        basePresentation = (baseProductTotalQty > 1) ? "cajas de parches" : "caja de parche";
    ...
    if(isIdeal && (productType == "euf" || productType == "hp" || productType == "vp_vials" || 
       ...
       (combinedType == "nothing" || combinedType == "") ) {
        ...
        basePresentation    += " " + baseTypeStr + " (" + baseMultiplier.toString(10);
        basePresentation    += ((baseMultiplier > 1) ? " cajas completas" : " caja completa") + ")";
    }
    else if(isIdeal && (productType == "euf" || productType == "hp" || productType == "vp_vials")) {
        basePresentation    += " " + baseTypeStr;
    }
    ...
    case 'pe':
        ...
        combinedPresentation        = (isIdeal) ? "cajitas" : "c&aacute;psulas";
        break;
    case 'vp_caps':
        combinedQty = 30;
        combinedTypeStr             = "MFIII VP";
        combinedPresentation        = "c&aacute;psulas";
        break;
    case 'parche_plus':
        combinedQty = 1;
        combinedTypeStr             = "Parche Pro-Sero";
        combinedPresentation        = "parches";
        break;
    case 'blue_cell':
        combinedQty = 1;
        combinedTypeStr             = "Blue Cell";
        combinedPresentation        = "frascos";
        break;
    case 'kimi':
        combinedQty = 1;
        combinedTypeStr             = "Kimi";
        combinedPresentation        = "plantillas";
        break;
    default:
        combinedTypeStr             = "";
    ...
    if(combinedPresentation == "ampollas") {
        combinedPresentation = (combinedQty > 1) ? "ampollas" : "ampolla";
    }
    if(combinedPresentation == "c&aacute;psulas") {
        combinedPresentation = (combinedQty > 1) ? "c&aacute;psulas" : "c&aacute;psula";
    }
    if(combinedPresentation == "frascos") {
        combinedPresentation = (combinedQty > 1) ? "frascos" : "frasco";
    }
    if(combinedPresentation == "plantillas") {
        combinedPresentation = (combinedQty > 1) ? "plantillas" : "plantilla";
    }
    if(combinedPresentation == "parches"){
        combinedPresentation = (combinedQty > 1) ? "parches" : "parche";
    }
    if(combinedPresentation == "cajitas")
        combinedPresentation = (combinedQty > 1) ? "cajitas" : "cajita";
    //Format string for combination:
    if(isIdeal) {
        combinedPresentation    += " " + combinedTypeStr;
    }
    ...
    if(isIdeal) {
        ...
        return baseProductTotalQty.toString(10) + " " + basePresentation + combinedFinal + " al mes";
    }
}
